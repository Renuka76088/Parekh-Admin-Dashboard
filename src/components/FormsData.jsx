...
const FormsData = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      setData({ trade, quotation: quot, auction: auc, appointment: appt, buyer, seller, contact, bulk });
    } catch (error) {
                            ...
                            <tr key={item._id} className="group hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-8 py-5">
                                    <div className="flex flex-col">
                                        <span className="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">
                                            {item.traderName || item.visitorName || item.buyerName || item.sellerName || item.participantName || item.name || 'Anonymous Sender'}
                                        </span>
                                ...
                                <td className="px-8 py-5 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        {(item.gstCertificate || item.proofFile || (item.kycDocuments && item.kycDocuments[0])) ? (
                                            <a 
