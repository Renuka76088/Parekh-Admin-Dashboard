...
import { equotationApi, equotationHeaderApi } from '../utils/api';
...
const EQuotation = () => {
  ...
  const fetchHeader = async () => {
    try {
      const res = await equotationHeaderApi.get(selectedWebsite);
  ...
  const handleHeaderSubmit = async (e) => {
    ...
    try {
      ...
      await equotationHeaderApi.update(selectedWebsite, headerData);
      ...
      <form onSubmit={handleHeaderSubmit} className="premium-card p-6 md:p-8 space-y-6 bg-white border border-slate-100">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">Modify the title and description shown on the user portal's E-Quotation page.</p>
          </div>
        </div>
